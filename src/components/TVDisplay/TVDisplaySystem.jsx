  ...
  const fetchTVData = useCallback(async () => {
    try {
      ...
      const [activeRes, completedRes] = await Promise.all([
        fetch(`${config.API_BASE_URL}/api/tv/current-services`, { headers, signal: controller.signal }),
        fetch(`${config.API_BASE_URL}/api/washes?status=completed&date=today`, { headers, signal: controller.signal }),
      ]);
