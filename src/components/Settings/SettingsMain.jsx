                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => {
                          const newPrice = Math.max(0, currentService.basePrice - 1);
                          setServiceConfig(prev => ({
                            ...prev,
                            [key]: { ...currentService, basePrice: newPrice }
                          }));
                        }}
                        className={`w-10 h-10 rounded-lg bg-red-500/20 text-red-600 hover:bg-red-500/30 transition-all duration-200 hover:scale-110 shadow-sm flex items-center justify-center font-bold text-lg ${currentTheme.ring} focus:outline-none focus:ring-2`}
                      >
                        −
                      </button>
                      
                      <div className={`flex-1 px-4 py-3 rounded-lg ${currentTheme.glass} ${currentTheme.border} text-center relative`}>
                        <span className={`font-bold text-lg ${currentTheme.text}`}>
                          {currentService.basePrice}
                        </span>
                        <span className={`absolute right-3 top-1/2 transform -translate-y-1/2 ${currentTheme.textMuted} text-sm font-medium`}>
                          DT
                        </span>
                      </div>
                      ...
                      <button
                        onClick={() => {
                          const newPrice = Math.min(1000, currentService.basePrice + 1);
                          setServiceConfig(prev => ({
                            ...prev,
                            [key]: { ...currentService, basePrice: newPrice }
                          }));
                        }}
                        className={`w-10 h-10 rounded-lg bg-green-500/20 text-green-600 hover:bg-green-500/30 transition-all duration-200 hover:scale-110 shadow-sm flex items-center justify-center font-bold text-lg ${currentTheme.ring} focus:outline-none focus:ring-2`}
                      >
                        +
                      </button>
                    </div>
